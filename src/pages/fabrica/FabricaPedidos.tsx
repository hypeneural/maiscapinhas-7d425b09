/**
 * Factory Orders List Page
 * 
 * Factory portal page to list and filter pending orders.
 */

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Factory, Eye, Calendar, Bell, Loader2 } from 'lucide-react';
...
import { PedidoProducaoStatus } from '@/components/producao';
import { useFabricaPedidos } from '@/hooks/api/use-fabrica';
import {
    PRODUCAO_STATUS,
    PRODUCAO_STATUS_LABELS,
    type FabricaPedidoFilters,
    type ProducaoPedido,
...
export function FabricaPedidos() {
    ...
    const [filters, setFilters] = useState<FabricaPedidoFilters>({
        page: 1,
        per_page: 10,
    });

    const { data, isLoading, error } = useFabricaPedidos(filters);

    const handleFilterChange = (key: keyof FabricaPedidoFilters, value: unknown) => {
        setFilters((prev) => ({
            ...prev,
            [key]: value,
    ...
    const newOrdersCount = pedidos.filter(
        (p) => p.status === PRODUCAO_STATUS.ENCOMENDA_REALIZADA
    ).length;
                ...
                <div className="space-y-4">
                    {pedidos.map((pedido) => (
                        <FactoryOrderCard
                            key={pedido.id}
                            pedido={pedido}
                            onClick={() => navigate(`/fabrica/pedidos/${pedido.id}`)}
                        />
                    ))}
                </div>
            )}
...
// Factory Order Card Component
function FactoryOrderCard({ pedido, onClick }: { pedido: ProducaoPedido; onClick: () => void }) {
    const isNew = pedido.status === PRODUCAO_STATUS.ENCOMENDA_REALIZADA;

    return (
        <Card
            className={`hover:bg-accent/50 transition-colors cursor-pointer ${isNew ? 'ring-2 ring-orange-500/50' : ''}`}
            onClick={onClick}
        >
            <CardContent className="p-4">
                <div className="flex items-center justify-between gap-4">
                    ...
                    <Button variant={isNew ? 'default' : 'ghost'} size="sm">
                        <Eye className="h-4 w-4 mr-2" />
                        {isNew ? 'Ver e Aceitar' : 'Ver'}
                    </Button>
                </div>
            </CardContent>
        </Card>
